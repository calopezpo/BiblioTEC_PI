...
@app.route("/")
def inicio():
    libros = MaterialBibliografico.query.filter_by(estado="Activo").order_by(MaterialBibliografico.id_libro.desc()).limit(6).all()
...
def catalogo():
    ...
    consulta = MaterialBibliografico.query.join(
        Autor, MaterialBibliografico.id_autor == Autor.id_autor
    ).join(
        Genero, MaterialBibliografico.id_genero == Genero.id_genero
    ).filter(
...
def sorprendeme():
    libro = MaterialBibliografico.query.filter_by(estado="Activo").order_by(func.random()).first()
...
def mood():
    ...
    if nombre:
        libros = MaterialBibliografico.query.join(
            LibroMood, LibroMood.id_libro == MaterialBibliografico.id_libro
        ).join(
            Mood, Mood.id_mood == LibroMood.id_mood
        ).filter(
            MaterialBibliografico.estado == "Activo",
            Mood.nombre.ilike(nombre)
        ).order_by(MaterialBibliografico.id_libro.desc()).all()
...
@app.route("/swipe")
def swipe():
    libros = MaterialBibliografico.query.filter_by(estado="Activo").order_by(MaterialBibliografico.id_libro.desc()).all()
...
def registro():
    if request.method == "POST":
        ...
        usuario = Usuario(nombres=nombres, apellidos=apellidos, correo=correo, rol="Lector")
...
def realizar_prestamo(id_ejemplar):
    ...
    try:
        ...
        ).order_by(Prestamo.id_prestamo.desc()).first()

        correo_enviado = False

        if prestamo:
            correo_enviado = enviar_correo_prestamo(prestamo)

        if correo_enviado:
            flash("Préstamo registrado. También enviamos la confirmación a tu correo.", "success")
        else:
            flash("Préstamo registrado correctamente, pero no se pudo enviar el correo.", "warning")

    except SQLAlchemyError as error:
        db.session.rollback()
        print("ERROR PRESTAMO:", error)
...
def procesar_devolucion(prestamo):
    ...
    return enviar_correo_devolucion(prestamo)
...
def devolver_prestamo(id_prestamo):
    ...
    try:
        correo_enviado = procesar_devolucion(prestamo)

        if correo_enviado:
            flash("Libro devuelto correctamente. Te enviamos la confirmación por correo.", "success")
        else:
            flash("Libro devuelto correctamente, pero no se pudo enviar el correo.", "warning")

    except SQLAlchemyError as error:
        db.session.rollback()
        print("ERROR DEVOLUCION:", error)
...
def realizar_reserva(id_libro, id_biblioteca):
    ...
    try:
        db.session.execute(
            text("CALL sp_realizar_reserva(:usuario, :libro, :biblioteca)"),
            {
                "usuario": session["usuario_id"],
                "libro": id_libro,
...
def mis_libros():
    ...
    prestamos = Prestamo.query.filter_by(id_usuario=usuario_id).order_by(Prestamo.id_prestamo.desc()).all()
    reservas = Reserva.query.filter_by(id_usuario=usuario_id).order_by(Reserva.id_reserva.desc()).all()
...
def admin():
    ...
    autores = Autor.query.order_by(Autor.apellidos, Autor.nombres).all()
    generos = Genero.query.order_by(Genero.nombre).all()
    moods = Mood.query.order_by(Mood.nombre).all()
    bibliotecas = Biblioteca.query.filter_by(estado="Activa").order_by(Biblioteca.nombre).all()
    prestamos_activos = Prestamo.query.filter_by(estado="Activo").order_by(Prestamo.fecha_limite.asc()).all()
...
def admin_nuevo_libro():
    ...
    portada = request.form.get("portada", "").strip() or None
    formato = request.form.get("formato", "Fisico").strip()

    id_autor = request.form.get("id_autor", type=int)
    nuevo_autor_nombres = request.form.get("nuevo_autor_nombres", "").strip()
    nuevo_autor_apellidos = request.form.get("nuevo_autor_apellidos", "").strip()

    id_genero = request.form.get("id_genero", type=int)
    id_mood = request.form.get("id_mood", type=int)

    anio_texto = request.form.get("anio_publicacion", "").strip()
    anio_publicacion = int(anio_texto) if anio_texto.isdigit() else None

    id_biblioteca = request.form.get("id_biblioteca", type=int)
    ...
    if not titulo or not isbn or not id_genero:
        flash("Título, ISBN y género son obligatorios.", "warning")
        return redirect(url_for("admin"))

    if formato not in ("Fisico", "Digital", "Audiolibro"):
        flash("El formato seleccionado no es válido.", "warning")
        return redirect(url_for("admin"))

    if MaterialBibliografico.query.filter_by(isbn=isbn).first():
        flash("Ya existe un libro con ese ISBN.", "danger")
        return redirect(url_for("admin"))

    if formato == "Fisico" and codigo:
        if not id_biblioteca:
            flash("Para crear el ejemplar físico debes elegir una biblioteca.", "warning")
            return redirect(url_for("admin"))

        if Ejemplar.query.filter_by(codigo=codigo).first():
    ...
    try:
        if not id_autor:
            if not nuevo_autor_nombres or not nuevo_autor_apellidos:
                flash("Selecciona un autor existente o escribe el nombre y apellido del nuevo autor.", "warning")
                return redirect(url_for("admin"))

            autor_existente = Autor.query.filter(
                func.lower(Autor.nombres) == nuevo_autor_nombres.lower(),
                func.lower(Autor.apellidos) == nuevo_autor_apellidos.lower()
            ).first()
            ...
            if autor_existente:
                ...
                db.session.execute(
                    text("CALL sp_registrar_autor(:nombres, :apellidos, :nacionalidad)"),
                    {
                ...
                if not nuevo_autor:
                    db.session.rollback()
                    flash("No se pudo registrar el nuevo autor.", "danger")
                    return redirect(url_for("admin"))
        ...
        db.session.execute(
            text("CALL sp_registrar_libro(:titulo, :isbn, :descripcion, :anio, :portada, :formato, :autor, :genero)"),
            {
                "titulo": titulo,
                "isbn": isbn,
        ...
        libro = MaterialBibliografico.query.filter_by(isbn=isbn).first()

        if not libro:
            db.session.rollback()
            flash("No se pudo encontrar el libro después de registrarlo.", "danger")
            return redirect(url_for("admin"))


        # Si no se selecciona Mood, se asigna automaticamente segun el genero
        if not id_mood:
            genero_libro = db.session.get(Genero, id_genero)
            ...
            if genero_libro:
                ...
                nombre_mood = relaciones_mood.get(genero_nombre)

                if nombre_mood:
                    mood_automatico = Mood.query.filter(func.lower(Mood.nombre) == nombre_mood.lower()).first()
        ...
        if id_mood:
            ...
            if not relacion_existente:
                db.session.execute(
                    text("CALL sp_asignar_mood_libro(:libro, :mood)"),
                    {
        ...
        if formato == "Fisico" and codigo:
            db.session.execute(
                text("CALL sp_registrar_ejemplar(:libro, :biblioteca, :codigo, :pasillo, :estante)"),
                {
                    "libro": libro.id_libro,
                    "biblioteca": id_biblioteca,
                    "codigo": codigo,
        ...
        flash("Libro agregado correctamente al catálogo y Mood Search.", "success")
        ...
        flash("No se pudo agregar el libro. Revisa los datos ingresados.", "danger")
...
def admin_devolver_prestamo(id_prestamo):
    ...
    if prestamo.estado != "Activo":
        flash("Ese préstamo ya no está activo.", "warning")
        return redirect(url_for("admin"))
    ...
    try:
        ...
        if correo_enviado:
            flash("Préstamo marcado como devuelto y usuario notificado por correo.", "success")
        else:
            flash("Préstamo devuelto, pero el correo no pudo enviarse.", "warning")

    except SQLAlchemyError as error:
        db.session.rollback()

        print("ERROR DEVOLUCION ADMIN:", error)

        flash("No se pudo completar la devolución.", "danger")
...
def error_interno(error):
    db.session.rollback()
    return render_template("error.html", codigo=500, mensaje="Ocurrió un error interno en BiblioTEC."), 500
